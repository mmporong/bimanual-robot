...
450×340 mm 상·하부 프레임, 4분할 상판, 720 mm 작업 높이, SO-101 두 대, 왼쪽 스톡 회전식 죠 형상 프록시, 오른쪽 ggao50 평행 그리퍼 프록시, Astra S, LDS-03, 2륜+2볼캐스터를 하나의 TF 트리로 묶는 ROS 2 Jazzy 설명 패키지다. 실물 왼손은 80 mm·립 3 mm TPU FinRay 손가락으로 선정됐지만, 이 패키지의 왼손 형상은 라이선스 확인 또는 프로젝트 작성 프록시가 준비될 때까지 스톡 죠를 유지한다.
