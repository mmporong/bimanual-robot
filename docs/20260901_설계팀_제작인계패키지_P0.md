...
| G-06 평행그리퍼 | Robonine 원형 170 g은 목표 130 g 초과 | 완성 질량, 손목축 기준 COM, 파지 반복시험 | 본수집형 `≤130 g`, COM `≤65 mm` |
...
문서의 관절 후보와 그리퍼 목표좌표는 로컬 기준 파일 `$HOME/jdamr_cube_ws/src/jdamr_cube_ros/capstone_pick/capstone_pick/kinematics.py`가 사용하는 JD-AMR의 SO-101 체인에서 나왔다. 이 체인은 `arm_shoulder_pan/lift/elbow_flex/wrist_flex/wrist_roll`, 자체 TCP, 대칭에 가까운 관절 한계를 사용한다. 현재 `hold_flow.urdf`는 Robonine의 전체 팔 Xacro를 벤더링했고 `link1_1` 계열 이름, 다른 joint origin·axis·zero·limit를 사용한다. 평행그리퍼 형상만 가져오려던 목적과 달리 팔 체인 규약까지 바뀐 상태다.

권고안은 다음과 같다.

1. **우선안:** 실기체에서 이미 검증한 JD-AMR/SO-101 arm chain을 좌우 prefix 가능한 매크로로 가져오고, Robonine에서는 `link5 이후 평행 죠 기구와 메시`만 엔드이펙터로 부착한다.
...
대안은 Robonine 전체 팔 체인을 유지하고 모든 영점·한계·IK를 새로 만드는 것이다. 실기체 기준이 이미 있는 현재 프로젝트에서는 우선안이 검증 비용이 낮다. 어느 안이든 관절값의 부호만 임시로 뒤집어 통과 처리하지 않는다.
...
Robonine 평행그리퍼는 위 14종에 포함되지 않는다. 공개 원본의 STEP/STL·BOM·조립 구조를 기준으로 P0를 만들고, 이후 130 g 이하 경량 파생형을 설계한다.
...
| Robonine 170 g 원형+물체 | `5.342 kg` | P0 한정 |
| 붓기 동적 전방 여유 | `28.3 mm` | 설계 하한 20 mm 통과 |
| Robonine 원형 붓기 여유 | `24.1 mm` | 통과하나 여유 감소 |
| C018 바퀴당 필요토크, SF2 | `0.234~0.237 N·m` | 그리퍼 가정별 값, 정격 0.981 N·m의 23.8~24.2% |
| Robonine 원형+120 g 컵 팔 | `0.948 N·m` | 정격 여유 약 3.4%, P0만 허용 |
| Robonine 원형+140 g 컵 팔 | `1.003 N·m` | **정격 초과, 금지** |
| Astra+거치대 | `395.3 g` | 400 g 게이트 통과 |
| 마스트 3개+이음관 2개 | `504.3 g` | 1 N 변위 실물시험 필요 |

P0 계산은 250 mm 차체가 가능함을 보여 주지만 실물 통과를 대신하지 않는다. 최초 물체는 각각 120 g 이하로 시작한다. 140 g은 130 g 이하 경량 그리퍼와 전류·온도·안정성 시험을 통과한 뒤에만 허용한다. 200 g은 별도 단팔 조건시험, 300 g과 가득 찬 500 mL 병은 현재 자세에서 금지한다.
