...
## 2026-09-16 추가 적재 5 kg 예비 주행

현재 조립된 하단 차체에 **5 kg을 추가 적재**하고 웹 수동 조종기로 실내 평지에서 주행했다.
사용자 관찰상 주행이 완료됐고 눈에 띄는 속도 저하는 없었다. 이는 `5 kg 추가 적재` 결과이며
로봇을 포함한 총질량 5 kg이라는 뜻이 아니다.

이번 시험에서는 차체 자체 질량, 총질량, 명령 속도, 주행 시간·거리, 모터 전류·온도와 배터리
전압 강하를 계측하지 않았다. 따라서 결과는 **예비 통과**로 기록하며 정격 적재량, 연속 운전
적재량 또는 5 kg 초과 하중의 근거로 사용하지 않는다. 다음 하중 시험은 실물 질량을 먼저 재고
동일한 적재 위치에서 전류·온도·전압 강하·정지거리·캐스터와 허브 상태를 함께 기록한다.

### RGB-D 연결 위치

RGB-D 후보인 Orbbec Astra S는 General Driver, 모터 전원선 또는 LiDAR UART가 아니라 USB에
연결한다. 책상 위 단독 POC에서는 개발 노트북에 직접 연결해 장치·RGB·Depth·CameraInfo와
RTAB-Map 경로를 먼저 확인한다. 차체를 움직이는 탑재 시험에서는 Pi 4B의 USB-A 포트에 연결해
Pi가 드라이버·타임스탬프·ROS 토픽 발행을 맡고, 노트북이 무선으로 기록·RTAB-Map·3D 지도
생성·분석을 맡는다. Astra S 인터페이스는 USB 2.0이므로 파란 USB 3.0 소켓에 꽂아도 링크
속도는 USB 2.0이며, 실제 포트는 열거 후 `/dev/v4l/by-path`로 고정한다.

2026-09-16 파이에서 `lsusb`와 프로세스의 열린 장치를 대조한 결과, 베이스 제어보드는 40핀
헤더 UART `/dev/ttyS0`를 사용해 USB 포트를 점유하지 않았다. G4 LiDAR만 CP210x 브리지
`/dev/ttyUSB0`로 12 Mbps USB 연결을 사용했다. Pi 4B의 USB-A 4개 중 현재 ROS 하드웨어가
점유한 포트는 1개이고 3개가 비어 있으므로 Astra S를 연결할 자리가 있다. 카메라를 연결한 뒤에는 `lsusb -t`와
`/dev/v4l/by-path`를 다시 저장해 포트 이동이나 장치 번호 변경을 구분한다.
...
| 유효 트레드 | 0.1836 m | 복사 금지, 현재 실차 기하 0.510 m부터 실주행 보정 |
...
`design/mechanical/hold_flow_mechanical_v0_3.yaml`과 생성 URDF에는 명목 320 mm가 남아 있다.
현재 조립된 실차에는 사용자가 실측한 기하 중심거리 510 mm를 우선 적용한다. 이 값도 유효
트레드 보정 완료값은 아니므로 직진 거리와 제자리 회전각으로 보정해야 한다.
...
5. **M3 기구·오도메트리 보정:** `wheel_radius=0.0329 m`, 현재 실차 기하 `wheel_separation=0.510 m`는 시작값으로만 사용한다. 측정 구간의 직진 거리와 제자리 회전각으로 유효값을 보정하고, 반복 편차·전류·전압·온도·캐스터 떨림·정지거리를 기록한다. 실측 뒤 거리·각도 반복 오차와 보정 허용치를 동결하고 같은 조건을 다시 시험한다.
