...
작성: 2026-09-07
최종 갱신: 2026-09-08 — 산업형 `PLANNED`, 전체 `ACT`, phase별 `HYBRID` 비교 구조 반영
...
- **후속 구현 결정**: 회의가 정한 태스크를 바꾸지 않고, 이후 조사·사용자 지시로 확정한
  구현·평가 방법
...
3. 주방·테이블 조작의 `control_strategy`, phase별 backend·checkpoint·입력·결과가 남는다.
...
flowchart LR
    ...
    MISSION --> P1["주방 조작 router<br/>PLANNED | ACT | HYBRID"]
    MISSION --> P2["테이블 조작 router<br/>PLANNED | ACT | HYBRID"]
    MISSION --> DOCK[Nav2 Docking Server]
    CAM[Astra S RGB-D] --> ALIGN
    CAM --> FILL["YOLO segmentation<br/>액면·흘림 판정"]
    FILL --> MISSION
    P1 --> PLAN[비전·IK·검증 궤적<br/>폐루프 제어]
    P1 --> ACT[phase별 ACT]
    P2 --> PLAN
    P2 --> ACT
    PLAN --> MUX[팔 명령 중재기]
...
- Nav2는 이동만 담당한다. `PLANNED`와 `ACT`는 베이스가 정지한 조작 구간에서만 실행한다.
...
- 양팔 조작은 `PLANNED_ALL`, `ACT_ALL`, `HYBRID` 세 전략을 같은 조건에서 비교한다.
- `PLANNED`는 RGB-D·그립 계산·IK·검증 궤적·그리퍼 피드백·액면 폐루프를 묶은 산업형
  기준선이다. IK만으로 전체 공정을 해결한다는 뜻이 아니다.
- `ACT`는 양팔이라는 이유만으로 채택하지 않는다. 같은 phase의 PLANNED 기준선보다
  일반화·복구·변경 비용을 개선한 경우에만 배포 backend로 선택한다.
- `HYBRID`는 phase 경계에서 두 backend를 순차 전환한다. 같은 관절에 동시에 명령하거나
  움직이는 중간에 backend를 바꾸지 않는다.
...
| PLANNED 조작 | 구형 역할의 IK 도구만 존재 | 새 팔 역할, phase 경계, 그립·붓기 폐루프 구현 |
| ACT | 리서치·데이터 규격 존재, 현행 태스크 데이터 없음 | phase 표시 smoke dataset과 비교 checkpoint 생성 |
...
### 4.4 주방 조작 policy 1 — PLANNED·ACT·HYBRID 비교

**회의에서 확정된 동작 순서**는 유지한다.
...
`policy 1`은 이 주방 조작 묶음의 미션 ID다. 이후 구현 결정으로 실행기를 ACT 하나로
고정하지 않고 다음 phase와 backend로 분리한다.

| phase | PLANNED backend | ACT backend |
|---|---|---|
| `CUP_PICK` | RGB-D 그립 자세→IK 접근→그리퍼 피드백→들기 | `ACT_CUP_PICK` |
| `JUG_PICK` | RGB-D 그립 자세→IK 접근→그리퍼 피드백→들기 | `ACT_JUG_PICK` |
| `MOVE_TO_PREPOUR` | 컵 기준 상대좌표와 검증 궤적 | `ACT_MOVE_TO_PREPOUR` |
| `POUR` | 기울임 기준 궤적 + 액면 폐루프 | `ACT_POUR` |
| `JUG_RETURN` | 직립·원위치 검증 궤적 | `ACT_JUG_RETURN` |
| `SHELF_PLACE` | 선반 목표 자세와 IK·그리퍼 이탈 | `ACT_SHELF_PLACE` |

세 비교 preset은 같은 상태기계와 안전 계층을 사용한다.

```yaml
planned_all:
  CUP_PICK: planned
  JUG_PICK: planned
  MOVE_TO_PREPOUR: planned
  POUR: planned
  JUG_RETURN: planned
  SHELF_PLACE: planned
...
`HYBRID`의 첫 후보는 산업형 방법으로 물체를 잡고 안전한 붓기 시작 자세까지 이동한 뒤,
양팔 붓기 미세조정만 ACT로 수행하는 구성이다. 실제 비교에서 ACT 파지가 위치·재질 변화에
더 강하면 마지막 접근·접촉 phase도 ACT로 바꿀 수 있다.

**공통 시작 게이트**

- 베이스 정지와 base controller 명령 lease 해제
- 컵·물통·선반 위치 관측 성공
- 양팔이 첫 phase 시작 자세 허용 범위 안
- 컵·물통 질량이 현재 페이로드 게이트 안
- 안전 게이트·카메라·선택 backend heartbeat 정상

**ACT 관측·행동 기준선**

- Astra S 고정 RGB 영상과 필요한 phase crop
- 왼쪽·오른쪽 팔의 5개 관절 위치와 그리퍼 상태
- 컵·물통·선반의 검출 결과 또는 상대 자세
- `phase_id`, 목표 물 양, 정규화한 액면 상태를 기록한다. ACT 입력 포함 여부는 별도
  ablation으로 비교하되 정확한 물 양 제어가 목표라면 영상만으로 종료 시점을 암묵적으로
  학습시키는 안보다 명시적 액면 상태를 우선 평가한다.
- 첫 행동 공간은 LeRobot `left_*`·`right_*` 절대 목표 위치로 고정한다.
...
```text
현재 backend 종료
  → 목표 자세 허용오차와 관절 속도 0 확인
  → 남은 ACT action chunk 폐기
  → 현재 command lease 해제
  → 다음 backend 상태 reset·시작 조건 확인
  → 다음 command lease 획득
  → 실행
```

IK와 ACT가 같은 관절에 동시에 명령하거나 움직이는 중간에 backend를 교체하지 않는다.
`q_command = q_planned + delta_q_learned` 형태의 residual 결합은 출력 의미가 다른 별도 정책과
안정성 검증이 필요하므로 첫 구현 범위에 넣지 않는다.

**전체 ACT와 HYBRID 데이터 관계**

전체 시연에는 phase timestamp를 함께 기록한다. 같은 원본을 전체 ACT 학습과 phase별 ACT
학습에 재사용할 수 있다. 다만 사람이 만든 시작 상태와 PLANNED backend가 만든 시작 상태가
다를 수 있으므로, HYBRID rollout에서 경계 분포 차이가 확인되면 해당 phase의 성공 시연이나
검토된 사람 개입 복구 구간만 보강한다. HYBRID 전체를 별도 모델로 처음부터 학습하지 않는다.

**ACT와 외부 감독기의 경계**

| ACT | 외부 감독기 |
|---|---|
| 선택된 phase의 영상·관절 상태에 따른 연속 동작 | 베이스 정지, 관절 한계, 충돌 거리, timeout |
| 붓기 자세·기울임·필요 시 컵 미세 안정화 | 물 양 독립 판정, 흘림 감시, 감속·중단, 실패 기록 |

ACT는 action chunk를 만들지만 `OVER`, `SPILL`, `UNKNOWN`, 충돌 위험에서는 남은 chunk를
폐기하고 검증된 감속·hold/stop으로 전환한다. 물 양은 정책의 자기평가가 아니라 YOLO 추정과
별도 계량 ground truth로 판정한다.

**단계별 검증**

1. **건식 P0**: PLANNED_ALL과 ACT_ALL이 같은 phase 순서·종료 자세를 만드는지 확인한다.
2. **고정량 P1**: 한 컵·한 목표량으로 PLANNED_ALL·ACT_ALL·HYBRID를 같은 protocol에서 비교한다.
3. **변동 P2**: 계획한 범위의 위치·조명·초기 잔량만 바꿔 holdout 성능과 안전 거부를 비교한다.
...
- 실행한 `control_strategy`와 phase별 backend·checkpoint가 기록됨
...
`PLANNED` 붓기에서는 이 결과를 기울임 감속·정지의 폐루프 입력으로 사용한다. `ACT`에서는
액면 상태를 정책 관측으로 넣는 안과 외부 감독기가 목표 도달 시 종료시키는 안을 분리해
평가한다. 어느 경우에도 `UNKNOWN`을 목표 도달로 간주하거나 추가 기울임을 허용하지 않는다.
YOLO 출력만으로 성공을 선언하지 않고 별도 계량 ground truth와 비교한다.
...
`policy 2`도 ACT 전용 이름이 아니라 테이블 조작의 미션 ID다. `PLANNED_ALL`은 선반·테이블
자세를 측정해 IK·검증 궤적으로 배치하고, `ACT_ALL`은 같은 구간의 phase checkpoint를
사용한다. 첫 `HYBRID`에서는 테이블 배치를 PLANNED로 두고, 테이블 높이·컵 위치·주변 장애물
변화에서 ACT가 실제 이점을 보일 때만 마지막 접근·놓기 phase를 교체한다.
...
| `ExecuteManipulationSkill` | Action | skill ID, strategy, phase별 backend·checkpoint, timeout / current phase / result | PLANNED·ACT·HYBRID 조작 |
...
`ExecuteManipulationSkill` 결과에는 최소한 다음이 필요하다.
...
## 6. PLANNED·ACT 비교 데이터와 실패 개선

### 6.1 정책 데이터셋을 나누는 기준

| dataset ID 예시 | 포함 | 제외 |
|---|---|---|
| `water_kitchen_full_demo_v1` | 주방 전체 시연과 모든 phase timestamp | Nav2 이동, 테이블 서빙 |
| `water_kitchen_pour_act_v1` | 원본에서 파생한 `POUR` 구간과 계보 | 다른 phase, 출처 없는 복사 |
| `water_table_full_demo_v1` | 선반 컵 파지부터 테이블 놓기·팔 복귀와 phase | 주방 붓기, Nav2 이동 |
| `water_kitchen_pour_hil_v1` | HYBRID 경계에서 검토된 사람 개입·복구 구간 | 원본 성공 시연과 무표시 혼합 |

전체 시연을 phase 표시 없이 하나의 긴 ACT에만 맞춰 수집하지 않는다. 원본의 phase 경계와
파생 데이터셋의 source episode·시간 범위를 남기면 같은 시연을 ACT_ALL과 로컬 ACT에
재사용할 수 있다. PLANNED가 만든 시작 상태에서만 생기는 분포 차이는 해당 phase 데이터로
보강한다.
...
- 원본 시연에는 `phase_id`, phase 시작·종료 timestamp와 성공 조건을 기록한다.
- rollout에는 `control_strategy`, phase별 `backend`, checkpoint ID, 캘리브레이션 해시,
  컵·물통·선반 버전을 sidecar에 남긴다.
- PLANNED 실행도 dataset·checkpoint 대신 planner·trajectory·controller·config 버전을 남겨
  ACT와 같은 수준으로 재현한다.

### 6.3 학습 순서

1. PLANNED_ALL로 phase 경계·성공 판정·안전 정지와 산업형 기준선을 만든다.
2. 시작 장면을 수동으로 맞춘 소수 smoke episode로 전체 로더·카메라·action key·phase 표시를 검증한다.
3. 같은 에피소드에 과적합해 ACT가 각 phase 또는 전체 순서를 재생 가능한지 확인한다.
4. 시작 위치·조명·물체 자세를 계획된 범위 안에서 나눠 수집한다.
5. ACT_ALL 기준선을 만들고 dataset·code·config·seed를 기록한다.
6. 같은 protocol에서 PLANNED_ALL·ACT_ALL·HYBRID를 holdout rollout한다.
7. 실패가 많은 phase와 확인된 원인에만 데이터를 보강한다.
...
실패마다 직전 영상, 관절·action window, strategy·backend·phase, YOLO 결과, 안전 게이트 상태를 묶는다.
“실패했다”는 메모만 남기지 않는다.
...
| 조작 전략 | `PLANNED_ALL / ACT_ALL / HYBRID`, phase별 backend·전환 성공·실패 |
...
세 전략은 같은 scenario matrix와 holdout split으로 평가한다. 위치·조명·초기 잔량·컵과
물통 조건을 전략마다 다르게 주지 않는다. ACT는 성공·일반화·복구 또는 변경 비용을 실제로
개선한 phase에만 채택하고, 차이가 없으면 PLANNED를 배포 기본값으로 둔다.
...
- 조작 Action 취소 뒤 실제 관절 명령이 멈췄는지 상태 피드백으로 확인한다.
...
| 사용자 `@mmporong` | PLANNED·ACT·HYBRID 공통 phase와 시작·복귀 자세 연결 | phase 경계·선반 좌표 |
| 사용자 `@mmporong` | 최종 모델 URDF 이후 Isaac Sim | 모델 URDF 전달·검증 완료 |
| 팀 policy lane | phase 표시 시연, ACT_ALL·로컬 ACT 학습·배포, 실패 개선 | policy 번호·사용 팔·phase 계약 |
...
| 조작 phase | strategy·backend, checkpoint 또는 planner·trajectory·config, 영상, 관측/action/관절 window, 그리퍼, 지연·지터, 안전 상태 | PLANNED·ACT·전환·하드웨어 실패 |
...
| 확인 결과 | 처리 |
|---|---|
| Nav2·TF·캘리브레이션·기구·서보 원인 | 시스템 수정 후 같은 checkpoint 또는 같은 PLANNED 산출물 버전으로 재시험 |
...
10. phase 이름·시작·종료 허용오차와 `PLANNED_ALL / ACT_ALL / HYBRID` scenario matrix
...
- [산업용 피킹과 IL·ACT 적용 경계](../research/R33_산업용_피킹과_IL_ACT_적용경계.md): 씨메스·ABB·FANUC·Dex-Net·ACT·RT-1·Figure를 대조한 적용 판정
