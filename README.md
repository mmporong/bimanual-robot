...
기본 실행은 위 CPU dry-run을 유지한다. 선택형 `ros2-mock` backend에서는 관제 `manipulate`
...
이 모드의 충전소 출발·복귀·충전은 mock이다. 선택형 `ros2-planned-roundtrip`은 충전소 출발부터
서빙 후 복귀까지 실제 바퀴 주행으로 연결하며, 도크 도착·정지 확인 뒤 주문을 성공 처리한다.
현재 1번 테이블 냉수 한 건의 물리 시험을 진행 중이다. 충전 물리·반복 주문 reset은 포함하지 않는다.
과거 접촉 프록시 입력과 최신 순정 그리퍼의 검증은 구분한다.
...
충전소 왕복 모드의 재현 절차와 범위는
[충전소 왕복 executor](docs/20260922_ExecuteManipulationSkill_ROS2_Action.md#10-충전소-왕복-executor)에 정리했다.
...
- PR 위험도는 검토 안내로만 쓰며, Draft가 아닌 PR은 `validate` 검사 통과 후 자동 병합한다.
- 분류·실패 원인·수정 및 재검증 절차는 Actions 실행 요약에만 기록하고 봇 댓글은 등록하지 않는다.
- 구형 `Auto PR`·`PR policy`는 비활성화했다. 오래된 브랜치는 main을 반영하면 새 자동 PR
  도우미를 사용한다. 이미 열린 PR은 다음 push·재개·Draft 해제 때 새 `PR assistant`가 처리한다.
- CODEOWNERS나 추가 승인 인원은 요구하지 않는다. PR 생성·병합·Actions 결과의 GitHub 기본
  알림은 수신자 설정에 따르며, 저장소에서 모든 수신자의 메일을 일괄 차단하는 기능은 없다.
