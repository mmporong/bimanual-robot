...
**SLAM/Nav2로 작업대까지 이동한 뒤, 한 팔은 경량 병을 기울이고 다른 팔은 받는 컵을 든다. 접촉 센서와 저울로 파지와 유입 결과를 판정하고, 실패하면 검증된 복구 스킬을 실행한다.**

2026-08-28 후속 결정으로 **이동형 비정형 용기 붓기를 Plan A이자 메인 시나리오로 확정**했다. 봉지 열어 담기는 메인 일정에서 빼고 후속 실험 후보로 보관한다.
...
- [x] 봉지·물병·접촉 센서·에이전틱 감독·모바일 양팔 논문 재검증
- [x] 물 따르기를 Plan A와 메인 시나리오로 확정
- [x] 물 따르기 전용 5쪽 발표자료와 디자인 리포트 제작
- [ ] 물병 총중량·파지·미끄럼 POC
- [ ] 컵 파지·건식 내용물 붓기·저울 판정 POC
