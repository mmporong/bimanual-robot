...
## 현재 최우선 작업 · 2026-09-21

실물 재개는 보류하고 [4테이블 식당 시뮬레이션](docs/20260921_식당_시뮬레이션_환경.md)을 검증했다.
자유 베이스에서 물을 붓고 중앙 받침에 컵을 놓은 뒤, 바퀴 속도로 운반해 손님 테이블에 내려놓는다.
가구·팔·컵·바닥 접촉을 검사하고, 테이블 지지와 그리퍼 열림·손 이격까지 확인한다.
주행은 시뮬레이터 좌표를 사용하는 A*·차동구동 추종이며 Nav2·ROS Behavior Tree는 아직 아니다.
목표 서빙 순서는 **물붓기 → 상판 중앙에 컵 내려놓기 → 손을 뺀 상태로 주행 → 컵 재파지 →
손님 테이블에 내려놓기**다. 컵을 손에 든 채 주행한 빈 컵 시험은 이 목표의 완료가 아니다.
기존 중앙 적재 자세의 자체 충돌을 확인한 뒤, 실험 사본에서 왼팔을 후방으로 20 mm 옮기고
200 mm 높이의 중앙 받침을 추가했다. 원본 설계는 유지하며 받침 제작·강성은 미검증이다.
정렬 제어의 왕복 문제를 수정한 `raised_service05`에서 **273.4초 조건 1회 연속 서빙을 통과했다.**
물붓기·중앙 적재·주행·재파지·손님 테이블 안착과 손 분리를 한 실행에서 확인했다.
최종 물은 컵 688개·병 230개·바깥 0개 입자이며, 운반 중 손실과 검출된 자체·환경 접촉은 0이다.
...
선정 FinRay STL은 해시·외곽 치수를 확인했으나 장착 변환·질량·라이선스가 미확정이어서
실제 형상으로 교체하지 않았다. 현재 접촉 결과는 강체 패드 모델에 한정한다.
...
사용하지 않는다. 왼손 강체 패드와 오른손의 추가 8 mm 장착 간격은 미보정 가정이므로
실물 물붓기 성공과 구분한다.
...
사용한다. URDF의 명목 빈 질량은 7.379 kg이며, 새 왼손 FinRay 손가락의 실측 질량은 아직
반영하지 않았다. 접합 강도, 동적 전도, 배선·방적과 실물 질량도 검증 전이다.
...
왼손은 SO-101 스톡 구동부를 유지하고 접촉 손가락을 **80 mm·끝단 립 3 mm TPU 95A FinRay 한 쌍**으로 교체했다. 출력 기준과 해시는 [`design/gripper/finray_80mm_lip3_selection.yaml`](design/gripper/finray_80mm_lip3_selection.yaml)에 있다. 2026-09-17 실측 원시 끝점 `1414~2871`에서 양 끝 40 tick을 제외한 `1454~2831`을 ID 6 사용 범위로 저장했다. 첫 컵 접근에서는 안정적인 파지·들어 올림을 확인하지 못했다. 컵 좌표 기준, FinRay TCP, 실제 추종 경로의 영향을 아직 분리하지 못했으므로 단일 원인을 확정하지 않는다. 젖은 컵 미끄럼 시험도 아직 수행하지 않았다. 출처 라이선스가 확인되지 않은 STL/G-code는 저장소에 포함하지 않는다.
