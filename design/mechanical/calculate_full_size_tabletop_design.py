#!/usr/bin/env python3
"""340×450 mm 4분할 상판·720 mm 작업 높이 모델의 기하/정역학 검사.
...
def mass_groups(components: list[dict]) -> dict[str, float]:
    """리뷰 가능한 수준으로 URDF 링크 질량을 기능별 집계한다."""
    groups = {
        "frame_and_tabletop": 0.0,
        "arms_and_grippers": 0.0,
        "battery_electronics_sensors": 0.0,
        "drive_and_casters": 0.0,
        "mounts_base_and_allowance": 0.0,
    }
    for component in components:
        name = component["name"]
        mass = component["mass_kg"]
        if "rail_link" in name or "seam_support" in name or "frame_column" in name or name.startswith("tabletop_"):
            groups["frame_and_tabletop"] += mass
        elif name.startswith(("left_", "right_")) and not name.endswith("wheel_link") and "backing" not in name:
            groups["arms_and_grippers"] += mass
        elif name in {"battery_link", "electronics_link", "camera_link", "laser_link"}:
            groups["battery_electronics_sensors"] += mass
        elif name.endswith("wheel_link") or name.endswith("caster_link"):
            groups["drive_and_casters"] += mass
        else:
            groups["mounts_base_and_allowance"] += mass
    return {name: round(mass, 3) for name, mass in groups.items()}
...
def main() -> None:
    ...
    grouped_mass = mass_groups(components)
    ...
    report = {
        ...
        "geometry": {
            ...
            "profile_ring_outer_xy_mm": [
                2.0 * float(ring["y_rail_center_x"]) + float(ring["section"][0]),
                2.0 * float(ring["x_rail_center_y"]) + float(ring["section"][1]),
            ],
        },
        "provisional_mass_model": {
            ...
            "groups_kg": grouped_mass,
            "task_mass_cases_kg": {
                "empty_robot": round(mass_kg, 3),
                "cup_0_30kg_plus_pitcher_0_56kg": round(mass_kg + 0.86, 3),
                "cup_0_30kg_plus_pitcher_1_10kg": round(mass_kg + 1.40, 3),
            },
            "configuration": "URDF articulated joint positions are zero",
            "scope": "URDF 명목 관성의 정적 검사이며 동적 전도·실물 하중 검증이 아님",
        },
        "gates": {
            "tabletop_matches_lower_frame_340x450": tabletop_xy == (340.0, 450.0),
            "profile_ring_outer_matches_340x450": (
                2.0 * float(ring["y_rail_center_x"]) + float(ring["section"][0]) == 340.0
                and 2.0 * float(ring["x_rail_center_y"]) + float(ring["section"][1]) == 450.0
            ),
            "tabletop_is_four_printable_panels": len(tabletop["quadrant_names"]) == 4,
            ...
            "arm_bases_are_150mm_from_front_edge": all(
                math.isclose(170.0 - float(arm["xyz"][0]), 150.0)
