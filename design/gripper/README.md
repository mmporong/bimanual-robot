...
현행 v0.3은 왼팔의 SO-101 스톡 구동부에 **80 mm·끝단 립 3 mm TPU 95A FinRay 손가락 두 개**를 장착하고, 오른팔에 ggao50 `SO101-Parallel-Gripper`를 쓴다. 왼쪽 컵 오버캡은 폐기하지 않고 이전 후보로 보존하며, 현행 선정값과 로컬 출력물 해시는 [`finray_80mm_lip3_selection.yaml`](finray_80mm_lip3_selection.yaml)에 둔다.

## 왼쪽 80 mm FinRay TPU 손가락
...
- 장착 뒤 그리퍼 캘리브레이션을 다시 하고, 완전 개폐 간섭·마른 컵·젖은 컵 미끄럼을 시험한다

원본 STL의 정확한 배포처와 라이선스를 기록하지 못했으므로 STL과 파생 G-code는 저장소에 넣지 않는다. 현재 URDF도 스톡 죠 프록시를 유지한다. 소스 라이선스를 확인하거나 같은 외곽의 프로젝트 작성 프록시를 만든 뒤 충돌 형상을 갱신한다.
...
|---|---|
| 파지면 평면 | `x = -49.4` |
| 블레이드 두께 `Y=-38` | 13.51 mm (`x -62.91 ~ -49.40`) |
| 블레이드 두께 `Y=-18` | 15.00 mm (`x -64.40 ~ -49.40`) |
| 구멍 | `Ø3.4`, `Y = -38 / -18`, `Z = 0`, X 방향 관통 |

`Z = 0` 중심선을 쓰는 이유는 홈이 병 축(`Z`)으로 파여도 중심선에는 항상 재료가 남기 때문이다. 구멍을 뚫은 뒤 두 죠 모두 watertight 이고 부피는 21.36 / 21.32 cm³ 다.
